...
class GraphTestCase(TestCase):
    ...
    def test_total_graph(self):
        ...
        self.assertEqual(response.status_code, 200)
        nodes = [dict['id'] for dict in graph['users']]
        self.assertEqual(len(nodes), 10)
        self.assertEqual(len(graph['friends']), 10)
        self.assertIn({'from': self.account1.id, 'to': self.account2.id}, graph['friends'])
        self.assertIn({'from': self.account2.id, 'to': self.account3.id}, graph['friends'])
        self.assertIn({'from': self.account2.id, 'to': self.account4.id}, graph['friends'])
        self.assertIn({'from': self.account3.id, 'to': self.account4.id}, graph['friends'])
        self.assertIn({'from': self.account3.id, 'to': self.account8.id}, graph['friends'])
        self.assertIn({'from': self.account4.id, 'to': self.account5.id}, graph['friends'])
        self.assertIn({'from': self.account4.id, 'to': self.account7.id}, graph['friends'])
        self.assertIn({'from': self.account5.id, 'to': self.account6.id}, graph['friends'])
        self.assertIn({'from': self.account6.id, 'to': self.account7.id}, graph['friends'])
        self.assertIn({'from': self.account7.id, 'to': self.account9.id}, graph['friends'])
    ...
    def test_level_graph(self):
        ...
        self.assertEqual(response.status_code, 200)
        graph = json.loads(response.content)
        nodes = sorted([dict['id'] for dict in graph['users']])
        edges = [sorted([edge['from'],edge['to']]) for edge in graph['friends']]
        self.assertEqual(nodes, sorted([self.account1.id, self.account2.id,
            self.account3.id, self.account4.id]))
        self.assertEqual(len(graph['friends']), 4)
        self.assertIn(sorted([self.account1.id, self.account2.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account3.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account4.id]), edges)
        self.assertIn(sorted([self.account3.id, self.account4.id]), edges)
        ...
        edges = [sorted([edge['from'],edge['to']]) for edge in graph['friends']]
        self.assertEqual(nodes, sorted([self.account1.id, self.account2.id,
            self.account3.id, self.account4.id, self.account5.id,
            self.account7.id, self.account8.id]))
        self.assertEqual(len(graph['friends']), 7)
        self.assertIn(sorted([self.account1.id, self.account2.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account3.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account4.id]), edges)
        self.assertIn(sorted([self.account3.id, self.account4.id]), edges)
        self.assertIn(sorted([self.account3.id, self.account8.id]), edges)
        self.assertIn(sorted([self.account4.id, self.account5.id]), edges)
        self.assertIn(sorted([self.account4.id, self.account7.id]), edges)

        response = client.get('/api/graph/3/')
        self.assertEqual(response.status_code, 200)
        graph = json.loads(response.content)
        nodes = sorted([dict['id'] for dict in graph['users']])
        edges = [sorted([edge['from'],edge['to']]) for edge in graph['friends']]
        self.assertEqual(nodes, sorted([self.account1.id, self.account2.id,
            self.account3.id, self.account4.id, self.account5.id, self.account6.id,
            self.account7.id, self.account8.id, self.account9.id]))
        self.assertEqual(len(graph['friends']), 10)
        self.assertIn(sorted([self.account1.id, self.account2.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account3.id]), edges)
        self.assertIn(sorted([self.account2.id, self.account4.id]), edges)
        ...
        self.assertIn(sorted([self.account4.id, self.account5.id]), edges)
        self.assertIn(sorted([self.account4.id, self.account7.id]), edges)
        self.assertIn(sorted([self.account5.id, self.account6.id]), edges)
        self.assertIn(sorted([self.account6.id, self.account7.id]), edges)
        self.assertIn(sorted([self.account7.id, self.account9.id]), edges)

        response = client.get('/api/graph/4/')
        self.assertEqual(response.status_code, 200)
        graph = json.loads(response.content)
        nodes = sorted([dict['id'] for dict in graph['users']])
        edges = [sorted([edge['from'],edge['to']]) for edge in graph['friends']]
